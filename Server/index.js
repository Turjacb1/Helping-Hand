...
mongoose.connect(process.env.MONGODB_URI || 'your-fallback-mongo-url', {
  useNewUrlParser: true,
  useUnifiedTopology: true,
})
  .then(() => console.log('MongoDB connected...'))
...
app.post('/addADoctor', (req, res) => {
  ...
  // Adjust the path to work on Heroku
  const uploadPath = path.join(__dirname, 'doctors', file.name);
  ...
  file.mv(uploadPath, async (err) => {
    ...
    try {
      await newDoctor.save();
      console.log('Doctor added successfully:', name);
      res.send({ name: file.name, path: `/doctors/${file.name}`, name, email });
    } catch (error) {
      console.error('Error adding doctor:', error);
      res.status(500).send({ msg: 'Failed to add doctor' });
    }
